...
public static class Utils
{
    ...
    public static byte[] HexToBytes(string hexString)
    {
        ...
        for (int i = 0; i < byteArray.Length; i++) byteArray[i] = Convert.ToByte(hexString.Substring(i * 2, 2), 16);
    ...
    public static string[] GenerateWords()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < 24; i++)
                words.Add(MnemonicWords.Bip0039En[
                    entropy[i] & 2047]); // We loose 5 out of 16 bits of entropy here, good enough
    ...
    public static byte[] GenerateSeedBip39(string[] mnemonic, string salt, int rounds, int keyLength)
    {
        ...
        KeyParameter?
            key = (KeyParameter)gen.GenerateDerivedMacParameters(keyLength *
                                                                 8); // note: keyLength is in bits, not bytes
        return key.GetKey();
    }
    ...
    public static KeyPair GenerateKeyPair(byte[] seed)
    {
        ...
        KeyPair keyPair = new(privateKeyBytes, publicKeyBytes);
    ...
    {
        return mnemonic.All(word => MnemonicWords.Bip0039En.Contains(word)) &&
               IsBasicSeed(MnemonicToEntropy(mnemonic.ToArray()));
    }
    ...
    static byte[] MnemonicToEntropy(string[] mnemonicArray, string password = "")
    {
        string mnemonicPhrase = string.Join(" ", mnemonicArray);

        return HmacSha512(mnemonicPhrase, password);
    }

    static byte[] Pbkdf2Sha512(byte[] password, string salt, int iterations)
    {
        Pkcs5S2ParametersGenerator generator = new(new Sha512Digest());
    ...
    static byte[] HmacSha512(string key, string data)
    {
    ...
    {
        Random random = new();
        ushort[] randomValues = new ushort[length];
        for (int i = 0; i < length; i++)
            randomValues[i] = (ushort)random.Next(ushort.MinValue, ushort.MaxValue + 1);
        return randomValues;
    }

    static BitArray DeriveChecksumBits(byte[] entropy)
    {
        int cs = entropy.Length * 8 / 32;
        SHA256 sha256 = SHA256.Create();
        byte[] hashValue = sha256.ComputeHash(entropy);
        BitArray bits = new(hashValue);
        BitArray slicedBits = new(cs);
        for (int i = 0; i < cs; i++) slicedBits[i] = bits[i];

        return slicedBits;
    }

    static BitArray BytesToBits(byte[] data)
    {
        BitArray bitArray = new(data);
        return bitArray;
    }

    static BitArray Concat(this BitArray _bits, BitArray newBits, bool inplace = false)
    {
        BitArray bits = inplace ? _bits : (BitArray)_bits.Clone();
        int offset = bits.Length;
        bits.Length += newBits.Length;
        return bits.Write(newBits, offset);
    }

    static BitArray Write(this BitArray _bits, BitArray newBits, int offset, bool inplace = true)
    {
        BitArray bits = inplace ? _bits : (BitArray)_bits.Clone();

        for (int i = 0; i < newBits.Length; i++)
            if (i + offset < bits.Length)
                bits[i + offset] = newBits[i];

        return bits;
    }

    static List<BitArray> SplitBitArray(BitArray bits, int chunkSize)
    {
        ...
        while (index < bits.Count)
        {
            ...
            BitArray chunk = new(chunkCount);

            for (int i = 0; i < chunkCount; i++) chunk[i] = bits[index + i];

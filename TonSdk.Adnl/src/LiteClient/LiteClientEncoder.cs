...
namespace TonSdk.Adnl.LiteClient
{
    ...
    {
        static (byte[], byte[]) EncodeBase(TLWriteBuffer methodWriter)
        {
                ...
                BitConverter.ToUInt32(
                    Crc32.ComputeChecksum(
                        Encoding.UTF8.GetBytes(
                            "liteServer.getMasterchainInfoExt mode:# = liteServer.MasterchainInfoExt")), 0));
        ...
        internal static (byte[], byte[]) EncodeGetBlockHeader(BlockIdExtended block)
        {
            ...
            writer.WriteUInt32(BitConverter.ToUInt32(Crc32.ComputeChecksum(
                Encoding.UTF8.GetBytes(
                    "liteServer.getBlockHeader id:tonNode.blockIdExt mode:# = liteServer.BlockHeader")), 0));
        ...
        internal static (byte[], byte[]) EncodeLookUpBlock(int workchain, long shard, long? seqno, ulong? lt,
            ulong? uTime)
        {
            TLWriteBuffer writer = new TLWriteBuffer();
            writer.WriteUInt32(BitConverter.ToUInt32(Crc32.ComputeChecksum(
        ...
        internal static (byte[], byte[]) EncodeGetConfigParams(BlockIdExtended block, int[] ids)
        {
            ...
            foreach (int item in ids) writer.WriteInt32(item);
        ...
        internal static (byte[], byte[]) EncodeGetLibraries(BigInteger[] list)
        {
            ...
            writer.WriteUInt32(BitConverter.ToUInt32(Crc32.ComputeChecksum(
                Encoding.UTF8.GetBytes(
                    "liteServer.getLibraries library_list:(vector int256) = liteServer.LibraryResult")), 0));

            writer.WriteUInt32((uint)list.Length);
            foreach (BigInteger t in list) writer.WriteInt256(t);
            return EncodeBase(writer);
        }
        ...
        internal static (byte[], byte[]) EncodeGetShardBlockProof(BlockIdExtended block)
        {
            ...
            writer.WriteUInt32(BitConverter.ToUInt32(Crc32.ComputeChecksum(
                Encoding.UTF8.GetBytes(
                    "liteServer.getShardBlockProof id:tonNode.blockIdExt = liteServer.ShardBlockProof")), 0));

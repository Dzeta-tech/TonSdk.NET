...
namespace TonSdk.Core.Boc
{
    public class Bits : IComparable<Bits>
    {
        ...
        public Bits(byte[] bytes)
        {
            ...
            for (int i = 0; i < _bytes.Length; i++) _bytes[i] = _bytes[i].reverseBits();

            Data = new BitArray(_bytes);
        }
        ...
        {
            BitArray _bits = (BitArray)Data.Clone();
        ...
        static BitArray fromString(string s)
        {
            ...
            {
                BitArray bits = new BitArray(bitString.Length);
            ...
            static BitArray fromHexString(string hexString)
            {
                ...
                {
                    BitArray bits = new BitArray(h.Length * 4);
                ...
                bool partialEnd = _hexString[_hexString.Length - 1] == '_';

                if (!partialEnd) return Parse(_hexString);

                _hexString = _hexString.Substring(0, _hexString.Length - 1);
                BitArray bits = Parse(_hexString);
        ...
        public string ToString(string mode)
        {
            ...
            {
                Bits _bits = new Bits(Data);
                int length = Data.Length;
                bool areDivisible = length % 4 == 0;
                BitArray augmented = areDivisible ? _bits.Data : _bits.Augment(4).Data;
                ...
                string newStr = new string(hexChars);

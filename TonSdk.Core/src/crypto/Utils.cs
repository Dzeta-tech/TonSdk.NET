...
namespace TonSdk.Core.Crypto
{
    public static class Utils
    {
        ...
        static BitArray Concat(this BitArray bits, BitArray newBits, bool inplace = false)
        {
            BitArray _bits = inplace ? bits : (BitArray)bits.Clone();
            int offset = _bits.Length;
            _bits.Length += newBits.Length;
            return _bits.Write(newBits, offset);
        }

        static BitArray Write(this BitArray bits, BitArray newBits, int offset, bool inplace = true)
        {
            BitArray _bits = inplace ? bits : (BitArray)bits.Clone();
